...
<thead>
   			<tr><th colspan="8"><h1>Documentos</h1></th></tr>
			<tr>
            ...
            <th scope="col" style='width:15%'>Fecha de alta</th>
...
$queryusuarios = mysqli_query($conn, "SELECT doc.iddocumento, alu.nombreapellido, alu.email, inst.nombre, doc.iddocumento, doc.documento, DATE_FORMAT(doc.fechaalta, '%m/%d/%Y') AS fechaalta, replace(doc.documento , ' ','') as nombredoc, doc.sellado, doc.archivo FROM alumnos as alu left join instituciones as inst ON alu.idinstitucion = inst.idinstitucion LEFT JOIN documentos AS doc ON alu.idalumno = doc.idalumno where alu.nombreapellido like '".$buscarna."%' and alu.email like '".$buscaremail."%' and inst.nombre like '".$buscarinst."%' and doc.documento like '".$buscardoc."%' and doc.fechaalta >= ifnull(STR_TO_DATE('".$buscardesde."','%d/%m/%y'),'1890-01-01') and doc.fechaalta < ifnull(STR_TO_DATE('".$buscarhasta."','%d/%m/%y'),'2999-01-01') and doc.estado = 0");
}
else
{
$queryusuarios = mysqli_query($conn, "SELECT doc.iddocumento, alu.nombreapellido, alu.email, inst.nombre, doc.iddocumento, doc.documento, DATE_FORMAT(doc.fechaalta, '%m/%d/%Y') AS fechaalta, replace(doc.documento , ' ','') as nombredoc, doc.sellado, doc.archivo FROM alumnos as alu left join instituciones as inst ON alu.idinstitucion = inst.idinstitucion LEFT JOIN documentos AS doc ON alu.idalumno = doc.idalumno where doc.estado = 0 ORDER BY alu.idalumno asc");
}
		$numerofila = 0;
...
<form action="sellar.php" onload="name_file();evento_file();" class="contenedor_popup" method="POST"> 
