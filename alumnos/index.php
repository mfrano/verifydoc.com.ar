...
<tr>
<th colspan="5"><button class="botondescargar2" style="font-family: FontAwesome;" name="btnbuscar" type="input" placeholder=" Filtrar Búsqueda" />Filtrar</th>
</tr>
</form>
</th>
</tr>
...
<tr>Agregar Nuevo Alumno<input style='font-family: FontAwesome;' placeholder='&#xf0fe' class='botondescargar' onclick="abrirform()"></input></tr> 
<thead>
</tr>
			<tr><th colspan="6"><h1>Alumnos</h1><th></tr>
			<tr>
		    <th>ID</th>
		    <th>DNI</th>
...
$queryusuarios = mysqli_query($conn, "SELECT t1.idalumno,t1.dni, t1.nombreapellido,t1.email, t2.nombre, DATE_FORMAT(t1.fechaalta, '%Y-%m-%d') AS fechaalta FROM alumnos AS t1 INNER JOIN instituciones AS t2  ON t1.idinstitucion = t2.idinstitucion   WHERE t1.nombreapellido like '".$buscarna."%'   AND t1.email like '".$buscaremail."%'   AND t2.nombre like '".$buscarinst."%' and t1.fechaalta >=  IFNULL(DATE_FORMAT('".$buscardesde."', '%Y-%m-%d'),'1890-01-01') AND t1.estado = 0");
/*$queryusuarios = mysqli_query($conn, "SELECT t1.idalumno,t1.dni, t1.nombreapellido,t1.email, t2.nombre, DATE_FORMAT(t1.fechaalta, '%Y-%m-%d') AS fechaalta FROM alumnos AS t1 INNER JOIN instituciones AS t2  ON t1.idinstitucion = t2.idinstitucion   WHERE t1.nombreapellido like '".$buscarna."%'   AND t1.email like '".$buscaremail."%'   AND t2.nombre like '".$buscarinst."%' and t1.fechaalta >= ifnull('".$buscardesde."','1890-01-01') and t1.fechaalta < ifnull('".$buscarhasta."','2999-01-01') AND t1.estado = 0");*/
/*$queryusuarios = mysqli_query($conn, "SELECT t1.idalumno,t1.dni, t1.nombreapellido,t1.email, t2.nombre, DATE_FORMAT(t1.fechaalta, '%Y-%m-%d') AS fechaalta FROM alumnos AS t1 INNER JOIN instituciones AS t2  ON t1.idinstitucion = t2.idinstitucion   WHERE t1.nombreapellido like '".$buscarna."%'   AND t1.email like '".$buscaremail."%'   AND t2.nombre like '".$buscarinst."%' AND t1.estado = 0");*/
}
else
{
$queryusuarios = mysqli_query($conn, "SELECT t1.idalumno,t1.dni, t1.nombreapellido,t1.email, t2.nombre, DATE_FORMAT(t1.fechaalta, '%Y-%m-%d') AS fechaalta FROM alumnos AS t1 INNER JOIN instituciones AS t2 ON t1.idinstitucion = t2.idinstitucion WHERE t1.estado = 0 ORDER BY idalumno asc");
}
		$numerofila = 0;
		...
		{    $numerofila++;    
            ...
            echo "<td style='width:26%'><a class='botondescargar' href=\"editar.php?cod=$mostrar[idalumno]\"><input style='font-family: FontAwesome;' placeholder='&#xf14b'  class='botondescargar'></input></a><a class='botondescargar' href=\"eliminar.php?codalumno=$mostrar[idalumno]\" onClick=\"return confirm('¿Estás seguro de eliminar el documento: $mostrar[nombreapellido]?')\"><input style='font-family: FontAwesome;' placeholder='&#xf1f8'  class='botondescargar'></input></a></td>";
}
?>
</tbody>
</table>
...
<div class="caja_popup" id="formregistrar">
  <form action="agregar.php" class="contenedor_popup" method="POST">
        <table>
		<tr><th colspan="2">Nuevo Alumnos</th></tr>
            <tr> 
     ...
     <?php endforeach; ?>
            ...
            <tr> 	
               <td colspan="2">
				   ...
				   <input type="submit" name="btnregistrar" value="Registrar" onClick="reload()">
			</td>
            </tr>
        </table>
    </form>
